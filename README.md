...
| residual-load forecast (load − solar − wind), **same-hour** | derived from the day-ahead forecasts above; the load thermal plants must cover — the dominant price driver, given explicitly because a tree cannot form the combination itself |
...
|---|---|---|---|
| **LightGBM + RES + weather + residual load (l1)** | **16.28** | **24.02** | **37.9** |
| LightGBM + RES forecast + weather | 16.85 | 24.67 | 37.8 |
...
**28% MAE improvement over the strongest baseline.**
...
> the deployed figures shift slightly month to month while the ~28% margin
> over naive holds.
...
- **Residual-load feature** (load forecast − day-ahead solar/wind forecast,
  the dominant price driver) is the latest win: 16.85 → 16.28 MAE (−3.3%,
  stable across seeds ±0.17). A gradient-boosted tree cannot form this linear
  combination from the separate inputs, so handing over the physical quantity
  directly helps. See `scripts/experiment_residual_tuning.py`.
...
TreeExplainer. Yesterday's price dominates (mean |SHAP| ≈ 19 EUR/MWh),
followed closely by the residual-load forecast (≈ 17), which absorbs most of
the separate solar, wind and load-forecast signal. The beeswarm confirms the
model learned the correct physics: high residual load pushes the predicted
price *up*, while a sunny, windy midday (low residual load) pushes it *down*
(the duck curve) — not a spurious correlation.
...
| υπολειπόμενο φορτίο (φορτίο − ηλιακή − αιολική), **ίδια ώρα** | παράγωγο των παραπάνω day-ahead προβλέψεων· το φορτίο που καλύπτουν οι θερμικές μονάδες — ο κυρίαρχος οδηγός τιμής, δοσμένο ρητά γιατί ένα δέντρο δεν σχηματίζει μόνο του τον συνδυασμό |
...
|---|---|---|---|
| **LightGBM + ΑΠΕ + καιρός + υπολειπόμενο φορτίο (l1)** | **16,28** | **24,02** | **37,9** |
| LightGBM + πρόβλεψη ΑΠΕ + καιρός | 16,85 | 24,67 | 37,8 |
...
**Βελτίωση 28% στο MAE από το ισχυρότερο baseline.**
...
> μετατοπίζονται ελαφρώς από μήνα σε μήνα ενώ το ~28% περιθώριο έναντι του
> naive διατηρείται.
...
- **Υπολειπόμενο φορτίο** (πρόβλεψη φορτίου − πρόβλεψη ηλιακής/αιολικής, ο
  κυρίαρχος οδηγός τιμής) είναι η πιο πρόσφατη βελτίωση: 16,85 → 16,28 MAE
  (−3,3%, σταθερό σε seeds ±0,17). Ένα δέντρο δεν μπορεί να σχηματίσει αυτόν
  τον γραμμικό συνδυασμό από τα ξεχωριστά inputs, οπότε δίνοντας απευθείας τη
  φυσική ποσότητα βοηθά. Βλ. `scripts/experiment_residual_tuning.py`.
...
TreeExplainer. Κυριαρχεί η χθεσινή τιμή (μέσο |SHAP| ≈ 19 EUR/MWh), ακολουθεί
στενά η πρόβλεψη υπολειπόμενου φορτίου (≈ 17), που απορροφά το μεγαλύτερο
μέρος του ξεχωριστού σήματος ηλιακής, αιολικής και φορτίου. Το beeswarm
επιβεβαιώνει τη σωστή φυσική: υψηλό υπολειπόμενο φορτίο ωθεί την τιμή *πάνω*,
ενώ ένα ηλιόλουστο/ανεμώδες μεσημέρι (χαμηλό υπολειπόμενο φορτίο) την ωθεί
*κάτω* (καμπύλη πάπιας) — όχι κάποια ψευδή συσχέτιση.
